...
export default class Bloomsky {
    ...
    if (api) {
      ...
      platform.api.on(
        ...
        function () {
          ...
          platform.updateData().catch((clientError) => {
  ...
  public configureAccessory(accessory: PlatformAccessory) {
    ...
    if (this.latestData != null && this.latestData.length > 0) {
      ...
      const stations = platform.latestData.filter(
        (station) =>
          accessory.UUID === Bloomsky.UUIDGen.generate(station.DeviceID)
      );
  ...
  public addAccessory(station: IStation) {
    ...
    if (
      newAccessory.services.length <= countBefore ||
    ...
    if (station.Data.DeviceType === "SKY1") {
      ...
      const batteryService = newAccessory.addService(
    ...
    let informationService = newAccessory.getService(
  ...
  public updateAccessory(station: IStation) {
    ...
    const accessory = platform.accessories.filter(
      (anyAccessory) => anyAccessory.UUID === stationUuid
    )[0] as PlatformAccessory;
    ...
    if (station.Data.DeviceType === "SKY1") {
      ...
      const batteryService = accessory.getService(
        Bloomsky.Service.BatteryService
      );
      batteryService
        .getCharacteristic(Bloomsky.Characteristic.BatteryLevel)
  ...
  private async updateData() {
    if (this.apiKey === undefined) {
      ...
      const response = await client
        .get<IStation[]>(this.apiUrl + "?unit=intl", requestOptions)
        .catch((clientError) => {
          if (platform.debug) {
            platform.log(clientError);
          }
      ...
      platform.timeout = setTimeout(
        function () {
          platform.updateData().catch((clientError) => {
            platform.log(clientError);
          });
        }.bind(platform),
        platform.debug ? 30000 : 150000
  ...
  private stationNeedsToBeRegistered(station: IStation) {
    ...
    return (
      this.accessories.filter(
        (accessory) => accessory.UUID.valueOf() === stationUuid
      ).length === 0
    );
  }
  ...
  private updateExistingAccessories() {
    ...
    const registeredAccessories = platform.latestData.filter(
      (station) => !this.stationNeedsToBeRegistered(station)
    );
  ...
  private registerNewAccessories() {
    ...
    const unregisteredAccessories = platform.latestData.filter((station) =>
      this.stationNeedsToBeRegistered(station)
    );
