...
var Bloomsky = /** @class */ (function () {
    ...
    if (api) {
      ...
      platform.api.on(
        ...
        function () {
          ...
          platform.updateData().catch(function (clientError) {
            platform.log(clientError);
          });
        }.bind(this)
      );
  ...
  Bloomsky.prototype.configureAccessory = function (accessory) {
    ...
    if (this.latestData != null && this.latestData.length > 0) {
      ...
      var stations = platform.latestData.filter(function (station) {
        return accessory.UUID === Bloomsky.UUIDGen.generate(station.DeviceID);
      });
  ...
  Bloomsky.prototype.addAccessory = function (station) {
    ...
    var newAccessory = new Bloomsky.Accessory(station.DeviceName, uuid);
    ...
    if (cameraSource.services.length === 0) {
      ...
      for (var _i = 0, _a = newAccessory.services; _i < _a.length; _i++) {
        ...
        if (this.debug) {
          this.log(
            service.UUID +
              (service.subtype !== undefined ? "." + service.subtype : "")
          );
        }
      }
    }
    if (newAccessory.services.length <= countBefore) {
      throw Error(
        "Camera Control Service Registration failed... +" +
    ...
    newAccessory
      .addService(Bloomsky.Service.TemperatureSensor, "Temperature")
      .getCharacteristic(Bloomsky.Characteristic.CurrentTemperature)
      .updateValue(station.Data.Temperature);
    newAccessory
      .addService(Bloomsky.Service.LightSensor, "Luminance")
      .getCharacteristic(Bloomsky.Characteristic.CurrentAmbientLightLevel)
      .updateValue(station.Data.Luminance);
    newAccessory
      .addService(Bloomsky.Service.HumiditySensor, "Humidity")
      .getCharacteristic(Bloomsky.Characteristic.CurrentRelativeHumidity)
      .updateValue(station.Data.Humidity);
    newAccessory
      .addService(Bloomsky.Service.MotionSensor, "Rain")
      .getCharacteristic(Bloomsky.Characteristic.MotionDetected)
      .updateValue(station.Data.Rain);
    newAccessory
      .addService(Bloomsky.Service.OccupancySensor, "Night")
      .getCharacteristic(Bloomsky.Characteristic.OccupancyDetected)
    ...
    if (station.Data.DeviceType === "SKY1") {
      ...
      var batteryLevel = Math.max(
        0,
        Math.min(100, (100 / 2600) * station.Data.Voltage)
      );
      var batteryService = newAccessory.addService(
        Bloomsky.Service.BatteryService,
        "Battery"
      );
      batteryService
        .getCharacteristic(Bloomsky.Characteristic.BatteryLevel)
        .updateValue(batteryLevel);
      batteryService
        .getCharacteristic(Bloomsky.Characteristic.ChargingState)
        .updateValue(2);
      batteryService
        .getCharacteristic(Bloomsky.Characteristic.StatusLowBattery)
        .updateValue(batteryLevel < 20);
    }
    var informationService = newAccessory.getService(
      Bloomsky.Service.AccessoryInformation
    );
    if (!informationService) {
      informationService = newAccessory.addService(
        Bloomsky.Service.AccessoryInformation,
        station.DeviceName
      );
    }
    informationService
      .getCharacteristic(Bloomsky.Characteristic.Manufacturer)
      .updateValue("Bloomsky");
    informationService
      .getCharacteristic(Bloomsky.Characteristic.Model)
      .updateValue(station.Data.DeviceType);
    informationService
      .getCharacteristic(Bloomsky.Characteristic.SerialNumber)
      .updateValue(station.DeviceID);
    informationService
      .getCharacteristic(Bloomsky.Characteristic.Identify)
      .updateValue(false);
    informationService
      .getCharacteristic(Bloomsky.Characteristic.Name)
      .updateValue(station.DeviceName);
    informationService
      .getCharacteristic(Bloomsky.Characteristic.FirmwareRevision)
      .updateValue(station.Data.DeviceType);
    if (this.debug) {
    ...
    if (this.api !== undefined) {
      ...
      this.api.publishCameraAccessories(
        "homebridge-bloomsky",
        this.accessories
      );
    } else {
  ...
  Bloomsky.prototype.updateAccessory = function (station) {
    ...
    var accessory = platform.accessories.filter(function (anyAccessory) {
      return anyAccessory.UUID === stationUuid;
    })[0];
    accessory
      .getService(Bloomsky.Service.TemperatureSensor)
      .getCharacteristic(Bloomsky.Characteristic.CurrentTemperature)
      .updateValue(station.Data.Temperature);
    accessory
      .getService(Bloomsky.Service.LightSensor)
      .getCharacteristic(Bloomsky.Characteristic.CurrentAmbientLightLevel)
      .updateValue(station.Data.Luminance);
    accessory
      .getService(Bloomsky.Service.HumiditySensor)
      .getCharacteristic(Bloomsky.Characteristic.CurrentRelativeHumidity)
      .updateValue(station.Data.Humidity);
    accessory
      .getService(Bloomsky.Service.MotionSensor)
      .getCharacteristic(Bloomsky.Characteristic.MotionDetected)
      .updateValue(station.Data.Rain);
    accessory
      .getService(Bloomsky.Service.OccupancySensor)
      .getCharacteristic(Bloomsky.Characteristic.OccupancyDetected)
      .updateValue(station.Data.Night);
    var informationService = accessory.getService(
      Bloomsky.Service.AccessoryInformation
    );
    informationService
      .getCharacteristic(Bloomsky.Characteristic.Manufacturer)
    ...
    if (station.Data.DeviceType === "SKY1") {
      ...
      var batteryService = accessory.getService(
        Bloomsky.Service.BatteryService
      );
      batteryService
        .getCharacteristic(Bloomsky.Characteristic.BatteryLevel)
    ...
    var file = fs.createWriteStream(
      this.temporaryFilenameForStationUuid(accessory.UUID)
    );
  ...
  Bloomsky.prototype.getFfmpegForStationWithUuid = function (uuid) {
    ...
    var ffmpeg = new Ffmpeg_1.FFMPEG(
      Bloomsky.UUIDGen,
      Bloomsky.Hap,
      {
        name: uuid,
      ...
      return __generator(this, function (_a) {
        switch (_a.label) {
          ...
          case 2:
            ...
            platform_1.timeout = setTimeout(
              function () {
                platform_1.updateData().catch(function (clientError) {
                  platform_1.log(clientError);
                });
              }.bind(platform_1),
              platform_1.debug ? 30000 : 150000
            ); // 2.5 minutes, 10 seconds for debugging
  ...
  Bloomsky.prototype.stationNeedsToBeRegistered = function (station) {
    ...
    return (
      this.accessories.filter(function (accessory) {
        return accessory.UUID.valueOf() === stationUuid;
      }).length === 0
    );
  };
  Bloomsky.prototype.updateExistingAccessories = function () {
    ...
    var registeredAccessories = platform.latestData.filter(function (station) {
      return !_this.stationNeedsToBeRegistered(station);
    });
  ...
  Bloomsky.prototype.registerNewAccessories = function () {
    ...
    var unregisteredAccessories = platform.latestData.filter(function (
      station
    ) {
      return _this.stationNeedsToBeRegistered(station);
    });
